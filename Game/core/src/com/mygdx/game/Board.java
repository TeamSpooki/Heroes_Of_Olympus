...
public class Board {
	...
	public Board() {
		...
		achille = new Hero(TextureRegion.split(new Texture(Gdx.files.internal("Achilles46.png")), 64, 64), "Achille", 1,
				1);
		...
		hercules = new Hero(TextureRegion.split(new Texture(Gdx.files.internal("Hercules46.png")), 64, 64), "Hercules",
				2, 3);
		hercules.setPosition(0, 320);
		board.put(new Location(0, 320), hercules);
		heroes.add(hercules);

		hypolyta = new Hero(TextureRegion.split(new Texture(Gdx.files.internal("Hippolyta46.png")), 64, 64), "Hypolyta",
				3, 2);
		hypolyta.setPosition(128, 448);
		board.put(new Location(128, 448), hypolyta);
		heroes.add(hypolyta);

		thesius = new Hero(TextureRegion.split(new Texture(Gdx.files.internal("Thesius46.png")), 64, 64), "Thesius", 1,
				15);
		...
		enemy4 = new Enemy(TextureRegion.split(new Texture(Gdx.files.internal("SkeletonSpear46.png")), 64, 64),
				"Enemy4");
		...
		enemy5 = new Enemy(TextureRegion.split(new Texture(Gdx.files.internal("SkeletonSpear46.png")), 64, 64),
				"Enemy5");
		...
		enemy6 = new Enemy(TextureRegion.split(new Texture(Gdx.files.internal("SkeletonSpear46.png")), 64, 64),
				"Enemy6");
	...
	public void carryOutAttacks() {
		// Go through the heroes
		for (GameUnit u : heroes) {
			// Go through the enemies
			for (GameUnit e : enemies) { // hero attacking is set to true
				if (u.isAttacking()) {
					// if the 'Y' position for both hero and enemy are the same
					if (isAlignedY(e, u)) {
						// set the damage to enemy
						e.setDamage(100);
	...
	public boolean isAlignedY(GameUnit a, GameUnit b) {
		return (a.getY() - b.getY()) < 64 && (a.getY() - b.getY()) > -64;
	}

	public void Draw(SpriteBatch batch) {

		Iterator<GameUnit> i = enemies.iterator();
		while (i.hasNext()) {
		GameUnit e = i.next(); // must be called before you can call i.remove()
